...
for(let i in chart_list){let html_id=undefined;try{html_id=chart_list[i].id}catch(error){console.log("No bloodmallet_chart was found.");return}
...
category_list=[];for(let i in dps_ordered_keys){let dps_key=dps_ordered_keys[i];category_list.push(get_category_name(state,dps_key,data))}
...
if(simulated_steps){for(let itemlevel_position in simulated_steps){let itemlevel=simulated_steps[itemlevel_position];var dps_array=[];for(let i in dps_ordered_keys){let dps_key=dps_ordered_keys[i];let dps_key_values=data.data[dps_key];if(Number(dps_key_values[itemlevel])>0){if(itemlevel_position==simulated_steps.length-1){if(itemlevel in dps_key_values){dps_array.push(dps_key_values[itemlevel]-baseline_dps)}else{dps_array.push(0)}}else{if(dps_key_values[simulated_steps[String(Number(itemlevel_position)+1)]]==0||!(simulated_steps[String(Number(itemlevel_position)+1)]in dps_key_values)){dps_array.push(dps_key_values[itemlevel]-baseline_dps)}else{dps_array.push(dps_key_values[itemlevel]-dps_key_values[simulated_steps[String(Number(itemlevel_position)+1)]])}}}else{if(itemlevel in dps_key_values){dps_array.push(dps_key_values[itemlevel])}else{dps_array.push(0)}}}
...
chart.addSeries({data:dps_array,name:itemlevel_clean,showInLegend:!0},!1)}}else{var dps_array=[];for(let i in dps_ordered_keys){let dps_key=dps_ordered_keys[i];let dps_key_values=data.data[dps_key];dps_array.push(dps_key_values)}
...
if(link.indexOf("item")>-1){if(data.hasOwnProperty("class_id")&&data.hasOwnProperty("used_azerite_traits_per_item")){link+="/azerite-powers=";link+=data.class_id;for(let i in data.used_azerite_traits_per_item[key]){const trait=data.used_azerite_traits_per_item[key][i];link+=":"+trait.id}}
...
link+="?itemLevel="+ilvl;if(data.hasOwnProperty("class_id")&&data.hasOwnProperty("used_azerite_traits_per_item")){link+="&azerite=";link+=data.class_id+":0";for(let i in data.used_azerite_traits_per_item[key]){const trait=data.used_azerite_traits_per_item[key][i];link+=":"+trait.id}}}
