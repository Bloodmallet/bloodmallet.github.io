...
const default_language="en";const dev_mode=!1;const path_to_data="https://bloodmallet.com/json/";let loaded_data={};this.init_charts=new function(){if(dev_mode){console.log("init_charts")}
...
function readd_wowdb_tooltips(chart_id){if(dev_mode){console.log("readd_wowdb_tooltips")}
...
link+=".wowhead.com/";try{let item_id=data.item_ids[key];link+="item="+item_id;link+="/"+slugify(key)}catch(error){if(dev_mode){console.log(error);console.log("We're probably looking at a spell.")}}
...
link+="&ilvl="+ilvl}
try{let spell_id=data.spell_ids[key];link+="spell="+spell_id;link+="/"+slugify(key)}catch(error){if(dev_mode){console.log(error);console.log("We're probably looking at an item.")}}
