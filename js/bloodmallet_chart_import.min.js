...
let chart_list=document.querySelectorAll("div.bloodmallet_chart");let tmp_id_list=[];for(const html_element of chart_list){if(tmp_id_list.indexOf(html_element.id)>-1){console.error(`Multiple Elements use the same ID ('${html_element.id}'). Aborting bloodmallet_chart_import.js.`);return}else{tmp_id_list.push(html_element.id)}}
for(const html_element of chart_list){let html_id=undefined;try{html_id=html_element.id}catch(error){console.log("No bloodmallet_chart was found.");return}
if(html_element){var state={data_entries:default_data_entries,data_type:default_data_type,azerite_tier:default_azerite_tier,fight_style:default_fight_style,background_color:default_background_color,font_color:default_font_color,axis_color:default_axis_color,tooltip_engine:default_tooltip_engine,chart_engine:default_chart_engine,wow_class:undefined,wow_spec:undefined};if(html_element.getAttribute("data-entries")){state.data_entries=html_element.getAttribute("data-entries")}
...
async function load_data(state){if(dev_mode){console.log("load_data")}
...
fetch(path_to_data+data_group+"/"+data_name).then(function(response){if(response.status!==200){console.warn('Problem occured when fetching data from bloodmallet.com. Status Code: '+response.status);return!1}
...
loaded_data[data_type][fight_style][wow_class][wow_spec]=json;if(dev_mode){console.log(json);console.log("Load and save finished.")}})}).catch(function(err){console.error('Fetching data from bloodmallet.com encountered an error, ',err)})}
...
category_list=[];for(let dps_key of dps_ordered_keys){category_list.push(get_category_name(state,dps_key,data))}
...
if(state.tooltip_engine!="wowhead"&&state.tooltip_engine!="wowdb"){return key}
if(state.data_type==="races"){return key}
if(state.tooltip_engine=="wowhead"){let link="<a href=\"https://www.wowhead.com/";try{let item_id=data.item_ids[key];link+="item="+item_id}catch(error){if(dev_mode){console.log(error);console.log("We're probably looking at a spell.")}}
