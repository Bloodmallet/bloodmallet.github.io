...
chart.addSeries({data:dps_array,name:itemlevel_clean,showInLegend:!0},!1)}}else{var dps_array=[];for(let dps_key of dps_ordered_keys){let dps_key_values=data.data[dps_key];dps_array.push(dps_key_values)}
chart.addSeries({data:dps_array,name:"Race",showInLegend:!0},!1)}
if(["trinkets","azerite_items_chest","azerite_items_head","azerite_items_shoulders","azerite_traits_itemlevel"].includes(data_type)){chart.legend.title.attr({text:"Itemlevel"})}else if(data_type==="races"){chart.legend.title.attr({text:""})}else if(data_type==="azerite_traits_stacking"){chart.legend.title.attr({text:"Trait count"})}
