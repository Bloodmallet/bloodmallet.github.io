...
data_name+="_"+fight_style;data_name+=".json";fetch(path_to_data+data_group+"/"+data_name).then(function(response){if(response.status!==200){console.warn('Problem occured when fetching data from bloodmallet.com. Status Code: '+response.status);return!1}
...
loaded_data[data_type][fight_style][wow_class][wow_spec]=json;console.log("Load and save finished.")})}).catch(function(err){console.error('Fetching data from bloodmallet.com encountered an error, ',err)})}
...
return}}else{dps_ordered_keys=data.sorted_data_keys.slice(0,data_entries);baseline_dps=data.data.baseline[data.simulated_steps[data.simulated_steps.length-1]]}
